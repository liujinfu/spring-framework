 ...
 * format). {@code <beans>} does not need to be the root element of the XML document: this class
 * will parse all bean definition elements in the XML file, regardless of the actual root element.
 *
 * @author Rod Johnson
...
public class DefaultBeanDefinitionDocumentReader implements BeanDefinitionDocumentReader {
	...
	protected void importBeanDefinitionResource(Element ele) {
		...
		try {
			absoluteLocation = ResourcePatternUtils.isUrl(location) || ResourceUtils.toURI(location)
					.isAbsolute();
		...
		if (absoluteLocation) {
			try {
				...
				if (logger.isTraceEnabled()) {
					logger.trace("Imported " + importCount + " bean definitions from URL location ["
							+ location + "]");
				}
			...
			try {
				...
				if (logger.isTraceEnabled()) {
					logger.trace(
							"Imported " + importCount + " bean definitions from relative location ["
									+ location + "]");
				}
